...
data Corners a = Corners {nw :: a, sw :: a, se :: a, ne :: a}
  deriving (Eq, Ord, Show, Generic, NFData, FromJSON, ToJSON, Functor) -- ne aka north east = upper left corner, etc

-- instance Foldable Corners where
--   foldr f b (Corners ne se sw nw) = foldr f b [ne, se, sw, nw]
